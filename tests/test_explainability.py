...
import numpy as np
import pytest
from synthefy_nori.explainability._common import (binarize01, binary_classes, clip_inf_edges,
                                                 detect_task, fill_nan, impute_mean, make_metric,
                                                 shape_direction, target_from_full, train_means)
...
def test_pipeline_run_end_to_end(use_test, tmp_path):
    ...
    from synthefy_nori.explainability.pipeline import run
    X, y, names = _toy()
    res = run(X[:120], y[:120], X[120:], y[120:], names, nori_model=Ridge(),
              reduce_threshold=3, out_dir=str(tmp_path), tag="toy",
              verbose=False, use_test=use_test)
...
def test_nori_interpreter_end_to_end(use_test):
    ...
    from synthefy_nori.explainability import NoriInterpreter
    X, y, names = _toy()
    interp = NoriInterpreter(model=Ridge(), reduce_threshold=3, render_figure=False,
                             use_test=use_test).fit(X, y + 1, feature_names=names)
...
def test_both_front_doors_agree_when_given_the_same_split():
    ...
    res = run(X[:140], y[:140], X[140:], y[140:], names, nori_model=Ridge(),
              reduce_threshold=2, verbose=False)
    interp = NoriInterpreter(model=Ridge(), reduce_threshold=2, render_figure=False,
                             test_size=0.3, random_state=0).fit(X, y, feature_names=names)
...
def test_load_table_reads_csv_and_parquet_identically(tmp_path):
    ...
    df = pd.DataFrame({"a": [1.0, 2, 3, 4, 5, 6, 7, 8], "b": list("xxyyxxyy"),
...
def test_interpreter_records_the_pruning_sweep():
    ...
    interp = NoriInterpreter(model=Ridge(), reduce_threshold=2, render_figure=False).fit(
