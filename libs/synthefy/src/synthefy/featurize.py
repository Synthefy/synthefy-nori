"""Shared client-side featurization of non-numeric DataFrame inputs.

The Synthefy client and legacy local Nori helpers accept Python lists, numpy
arrays, or pandas DataFrames. When **both** ``X_train`` and ``X_test`` are
DataFrames, this module turns any non-numeric columns into a fully numeric,
model-ready matrix client-side — fitting the encoding on ``X_train`` and applying
the same column layout to ``X_test`` — so raw categorical frames "just work"
without relying on the model to detect categories itself. The default
...
def _featurize_frames(
    ...
    *,
    ...
    if X_train_feat.shape[1] == 0 and not _allow_empty:
        raise ValueError(
