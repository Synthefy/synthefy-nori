...
def capture_policy_notes():
    ...
    - **Serialisation.** The filter state this swaps is process-global, so overlapping
      ...
      hold this INSIDE its inference lock -- see ``serving/core/nori_inference/engine.py``.
...
def forget_emitted_warnings() -> None:
    ...
    **long-lived server**, where each request carries a different caller's config —
    ...
    returns them to that request. ``serving/core/nori_inference/engine.py`` does exactly
...
class MemoryPolicy(BaseModel):
    """Controls memory used by Nori's per-request key/value cache.

    Large context tables can make the cache larger than the model itself. These fields set
    its precision and GPU/host budgets, and choose which lower-memory fallbacks are allowed.
    Every declared default is the value actually used. Unknown fields are rejected, and
    resolving it returns a new policy describing the fallback selected for one request.
    """
