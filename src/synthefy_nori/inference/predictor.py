...
class NoriPredictor:
    ...
    def __init__(
        ...
        # when training y has at most this many unique values.
        # DEFAULT OFF (0): the PFN-parity R² ablation showed snapping HURTS — on wine_quality
        # (6 discrete levels) it dropped R² 0.539 -> 0.455 (-0.084), because the
        # nearest seen level is more wrong than the model's continuous output.
        # Opt in per-deployment (>0) only with evidence it helps that target.
        ...
        if device_type == "cpu":
            ...
            # weights use float32. Preserve the released MPS quality contract.
            self.mix_precision = False
    ...
    def _maybe_snap_discrete_y(self, y_train: np.ndarray, preds: np.ndarray) -> np.ndarray:
        ...
        """
        try:
            # cuda-safe: preds / y_train may be torch tensors on GPU, which
            # np.asarray() cannot convert directly (raises "can't convert
            # cuda:0 device type tensor to numpy"). Detach to CPU first.
            def _to_np(x):
                return x.detach().cpu().numpy() if torch.is_tensor(x) else np.asarray(x)

            preds_np = _to_np(preds)
            y_arr = _to_np(y_train).astype(np.float64).reshape(-1)
            ...
            preds_arr = preds_np.reshape(-1).astype(np.float64)
            ...
            return chosen.reshape(preds_np.shape).astype(preds_np.dtype)
        except Exception as _e:
    ...
    def _bare_model(self):
        """Unwrap the DDP wrapper to reach the backbone module."""
    ...
    def _get_embeddings_impl(
        ...
        # Unwrap the DDP wrapper to reach the backbone for the embedding path.
        bare_model = self._bare_model()
    ...
    def _try_batched_ordinary_regression(
        ...
        *,
        ...
        cache.
        DDP, imputation, int8, host offload, pinned prefill chunking, and
