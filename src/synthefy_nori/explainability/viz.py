...
_DIR_LABEL = {"flat": "—", "negligible": "≈ flat", "increasing": "↑ increases target",
              "decreasing": "↓ decreases target", "non-monotone": "∿ mixed"}
...
def plot_ebm_model(model, feature_names, *, X_density=None, task="regression", target_name="target",
                   skill=None, skill_name=None, title=None, out_path=None, show_key=True,
                   feature_ranges=None, class_names=None):
    ...
    try:                                             # optional dep: the explainability extra
        ...
        raise ImportError("matplotlib is required to draw the model diagram; install it with "
    ...
    n_classes = int(np.asarray(_main0["scores"], float).shape[1]) if (
        _main0 is not None and np.asarray(_main0["scores"], float).ndim > 1) else 1
    ...
    if multiclass:
        ...
        y_desc = (f"how much that feature moves the score for EACH class — one curve per class, "
    ...
    for row, (t, feats, _) in zip(yr, main_terms):
        j = feats[0]
        f = fn[j]
        fig.text(in_x, row, f, ha="center", va="center", fontsize=13.5,
                 bbox=dict(boxstyle="round,pad=0.4", fc=fcolor[f], ec="black", alpha=.85))
        in_pos[t] = (in_x + in_w * 0.55, row)

    fig.text(mp_left + mp_w / 2, HDR_Y, "main-effect shape functions  fⱼ(xⱼ)",
             ha="center", fontsize=HDR_FS, weight="bold")
    for row, (t, feats, dta) in zip(yr, main_terms):
        ...
        curves = ([(ys[:, k], ccolor[k], class_names[k]) for k in range(n_classes)]
                  if multiclass else [(ys, fcolor[fn[j]], None)])
        keep = ([i for i in range(len(ys)) if xs[i + 1] > x0 and xs[i] < x1] if step
                else (xs[:len(ys)] >= x0) & (xs[:len(ys)] <= x1)) if frng else None
        visible = []
        for cy, colr, clabel in curves:
            if step:
                if not multiclass:
                    ax.stairs(cy, xs, fill=True, color=colr, alpha=.22)
        ...
        ax.axhline(0, color="k", lw=.5, alpha=.6)
        ax.set_xlim(x0, x1)
        ...
        if X_density is not None:
            ...
            if len(col):
                ...
                axd = ax.twinx()
                axd.bar((edg[:-1] + edg[1:]) / 2, cnt, width=(edg[1] - edg[0]), color="0.55", alpha=.30, zorder=0)
        ...
        ax.set_title(f"{fn[j]}   ·   imp {imp[t]:.2f} (#{rank_of[t]}/{n_terms}){tag}\n"
                     f"{trend}   ·   contribution ∈ [{ymin:+.2f}, {ymax:+.2f}]",
                     fontsize=12.5, pad=6, color=("0.45" if imp[t] < 0.05 * max_imp else "black"))
        fig.patches.append(FancyArrowPatch(in_pos[t], (mp_left, row), transform=fig.transFigure,
                           arrowstyle="-|>", mutation_scale=9, lw=.8, color=fcolor[fn[j]], alpha=.6, zorder=0))
        fig.patches.append(FancyArrowPatch((mp_left + mp_w, row), (sum_x - 0.03, sum_y), transform=fig.transFigure,
                           arrowstyle="-|>", mutation_scale=8, lw=.6, color="0.6", alpha=.5, zorder=0,
                           connectionstyle="arc3,rad=0.03"))

    ix_left, ix_w, ix_h = 0.500, 0.175, dyi(2.0)
    ix_top, ix_bot = yr[0], yr[-1]            # span the same vertical extent as the shape panels
    iyr = [ix_top - (ix_top - ix_bot) * (i / max(len(int_terms) - 1, 1)) for i in range(len(int_terms))] \
        if len(int_terms) > 1 else [(ix_top + ix_bot) / 2]
    if int_terms:
        shown = (f"top {len(int_terms)} of {len(all_int)}" if n_int_hidden else f"all {len(all_int)}")
        fig.text(ix_left + ix_w / 2, HDR_Y, "pairwise interactions  fⱼₖ(xⱼ,xₖ)",
                 ha="center", fontsize=HDR_FS, weight="bold")
        fig.text(ix_left + ix_w / 2, HDR_Y - dyi(0.30), f"({shown} — Σ below uses every one)",
                 ha="center", fontsize=10.0, color="0.35")
    for row, (t, feats, dta) in zip(iyr, int_terms):
        ...
        fig.patches.append(FancyArrowPatch((ix_left + ix_w, row), (sum_x - 0.03, sum_y), transform=fig.transFigure,
                           arrowstyle="-|>", mutation_scale=9, lw=.6, color="0.6", alpha=.5, zorder=0,
                           connectionstyle="arc3,rad=0.03"))
    if int_terms:
    ...
    fig.text(sum_x, sum_y, "Σ", ha="center", va="center", fontsize=40,
             bbox=dict(boxstyle="circle,pad=0.4", fc="#ffe08a", ec="black"))
    fig.text(sum_x, sum_y - dyi(2.3), f"intercept (baseline)\n{intercept:+.3f}", ha="center", va="center", fontsize=12,
             bbox=dict(boxstyle="round,pad=0.4", fc="white", ec="0.5"))
    fig.patches.append(FancyArrowPatch((sum_x, sum_y - dyi(1.7)), (sum_x, sum_y - dyi(0.6)), transform=fig.transFigure,
                       arrowstyle="-|>", mutation_scale=10, lw=.9, color="0.4"))
    if multiclass:
        # one Σ per class, then a softmax over the K sums
        out_eff = out_x + 0.02
        link_x = (sum_x + out_eff) / 2
        fig.patches.append(FancyArrowPatch((sum_x + 0.03, sum_y), (link_x - 0.019, sum_y),
                           transform=fig.transFigure, arrowstyle="-|>", mutation_scale=14, lw=1.6,
                           color="black"))
        fig.text(link_x, sum_y, "softmax", ha="center", va="center", fontsize=11,
                 bbox=dict(boxstyle="round,pad=0.34", fc="#ffd9b3", ec="black"))
        fig.patches.append(FancyArrowPatch((link_x + 0.026, sum_y), (out_eff - 0.032, sum_y),
                           transform=fig.transFigure, arrowstyle="-|>", mutation_scale=14, lw=1.6,
                           color="black"))
        fig.text((sum_x + link_x) / 2, sum_y + dyi(0.45), f"{n_classes} scores", ha="center",
                 fontsize=11, color="0.45")
        fig.text((link_x + out_eff) / 2, sum_y + dyi(0.45), "probabilities", ha="center",
                 fontsize=11, color="0.45")
        out_node_x = out_eff
    elif task == "classification":
        ...
        out_eff = out_x + 0.02
        link_x = (sum_x + out_eff) / 2
        fig.patches.append(FancyArrowPatch((sum_x + 0.03, sum_y), (link_x - 0.016, sum_y), transform=fig.transFigure,
                           arrowstyle="-|>", mutation_scale=14, lw=1.6, color="black"))
        fig.text(link_x, sum_y, "σ", ha="center", va="center", fontsize=21,
        ...
        fig.patches.append(FancyArrowPatch((link_x + 0.017, sum_y), (out_eff - 0.032, sum_y), transform=fig.transFigure,
                           arrowstyle="-|>", mutation_scale=14, lw=1.6, color="black"))
        ...
        fig.patches.append(FancyArrowPatch((sum_x + 0.034, sum_y), (out_x - 0.032, sum_y), transform=fig.transFigure,
                           arrowstyle="-|>", mutation_scale=16, lw=1.6, color="black"))
        out_node_x = out_x
    out_label = (f"ŷ\nP({target_name})\nper class" if multiclass else f"ŷ\n{target_name}")
    fig.text(out_node_x, sum_y, out_label, ha="center", va="center",
             fontsize=13 if multiclass else 16, weight="bold",
             bbox=dict(boxstyle="round,pad=0.5", fc="#c7e9c0", ec="black"))
    if multiclass:                       # one shared legend for the per-class curves
        handles = [plt.Line2D([], [], color=ccolor[k], lw=2.4, label=class_names[k])
                   for k in range(n_classes)]
        fig.legend(handles=handles, loc="upper right", bbox_to_anchor=(0.99, HDR_Y + dyi(0.5)),
                   frameon=True, fontsize=11, title="class", ncol=1)

    int_note = ("this model has no interaction terms (interpret's EBM does not support them for "
                "more than two classes)" if multiclass else
                f"the {MAX_INTERACTIONS_SHOWN} strongest of {len(all_int)} are drawn, but ALL "
                f"{len(all_int)} are in the model and in Σ" if n_int_hidden
    ...
    if show_key:
        ...
        key = (f"HOW TO READ  —  glass-box additive model.   Prediction = baseline + every feature's contribution.\n"
               ...
               f"(rank #1 = strongest).\n"
               + (f"• Interactions: {int_note}.   " if multiclass else
                  f"• Right heatmaps = pairwise interactions (effect of TWO features together beyond "
                  f"their curves; {heat_desc}); {int_note}.   ")
               + (f"• One Σ per class, then a softmax over the {n_classes} sums gives the "
                  f"predicted probabilities.   " if multiclass else
                  "• Σ combines the baseline and all contributions into the final prediction.   ")
               + f"{skill_txt}")
        fig.text(0.5, 0.088, key, ha="center", va="top", fontsize=9.0,
