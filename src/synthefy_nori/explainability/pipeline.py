...
from synthefy_nori.explainability._common import detect_task, encode_labels, make_metric, target_classes
from synthefy_nori.explainability._core import (
    SELECTION_FRACTION,
    SWEEP_FRACTIONS,
    distill_glassbox,
    prepare_rows,
...
def run(
    ...
    *,
    ...
    rows, _mu, _features_train, features_test = prepare_rows(
        ...
        yte,
        use_test=use_test,
        stratify=(ytr if task in ("classification", "multiclass") else None),
        rng=rng,
        nori_cap=nori_cap,
    ...
    log(
        f"[{tag}] selection stage: nfit={rows.n_select_fit} neval={rows.n_select_eval} "
        + (
            "on the TEST split (use_test=True; selection_at_n95 is a criterion value)"
            if use_test
            else "carved out of train (test split untouched until final scoring)"
        )
    )
    ...
    def importance_fn(fitted, eval_features, eval_target):
        ...
        if method == "shap":
            ...
            imp = nori_shap_importance(fitted, eval_features[q], bg, budget=budget, random_state=random_state)
        ...
        return nori_permutation_importance(
            fitted, eval_features, eval_target, metric, n_repeats=perm_repeats, random_state=random_state
        )
    ...
    ranked = [
        {"feature": names[j], "index": int(j), "importance": round(float(selection.importance[j]), 6)}
    ...
    ebm95, ebm95_skill, ebm_full, ebm_full_skill, fn95 = distill_glassbox(
        rows, selection, names=names, task=task, metric=metric, features_test=features_test, target_test=yte
    )
    ...
    res = {
        ...
        "importance": ranked,
        "nori_full": round(nori_full, 4),
        "target_95pct": round(selection.target, 4),
        ...
        "reduce_threshold": reduce_threshold,
        "reduced": selection.reduced,
        "n95": n95,
        "pct95": round(100 * n95 / d, 1),
        "nori_at_n95": round(nori_n95, 4),
        "selected_features": fn95,
        "selected_indices": [int(c) for c in cols95],
        "ebm_at_n95": round(ebm95_skill, 4),
        "ebm_full": round(ebm_full_skill, 4),
        "sweep": selection.curve,
        "ebm_model": ebm_structure(ebm95),
    }
    ...
    if out_dir:
        ...
        joblib.dump(
            {"model": ebm95, "feature_indices": [int(c) for c in cols95], "feature_names": fn95, "task": task}, jp
        )
    ...
    log(
        f"[{tag}] n95={n95} ({res['pct95']}% of {d})  Nori full={nori_full:+.3f} @n95={nori_n95:+.3f}"
...
def main():
    ...
    run(
        ...
        task=a.task,
        method=a.method,
        nori_model=a.nori_model,
        reduce_threshold=a.reduce_threshold,
        out_dir=a.out_dir,
        tag=tag,
    )
